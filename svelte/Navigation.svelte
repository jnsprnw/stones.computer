<script>
  ...
  $: currentActive = Math.max(
    compact(map(tops, (top) => top < y)).length - 1,
    0,
  );
  ...
  function getTops() {
    ...
    tops = map(
      anchors,
      ({ id }) => document.getElementById(id).offsetTop - offset,
    );
  }
...
<nav
  class:isBeginning={currentActive < 1}
  class:isLast={currentActive === anchors.length - 1}
>
  {#each anchors as { id, label }, i}
    <a
      ...
      class="internal"
      class:isActive={i === currentActive}
      ><i>{@html getIcon(i, currentActive)}</i> {label}</a
    >
  {/each}
  <a href="/archive" rel="search"
    ><i>↗</i> <strong class="archive">Archive</strong></a
  >
</nav>
