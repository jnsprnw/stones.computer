<script>
  ...
  // Rotation
  $: r = scaleLinear().domain([500, 1000]).range([90, 0]);
...
<ul class="map-topics">
  {#each TOPICS as topic}
    <li
      on:mouseover={() => handleoverTopic(topic)}
      on:focus={() => handleoverTopic(topic)}
      on:mouseleave={handleleaveTopic}
      on:blur={handleleaveTopic}
    >
      {topic}
    </li>
  {/each}
...
<svg
  bind:this={svg}
  class="map"
  class:hasActive={activeEdges.length || activeNodes.length || activeTopic}
  role="figure"
>
  <g>
    {#each links as { source, target, d }}
      <path
        {d}
        class="edge"
        class:isActive={activeEdges.some((edge) => {
          return edge[1] === source && edge[0] === target;
        })}
      />
    {/each}
  </g>
  <g>
    {#each aspects as { x, y, title, links, id, topics }}
      <a
        href="#{id}"
        on:click|preventDefault={() => scrollTo(id)}
        on:mouseover={() => handleoverAspect([id, ...links], id)}
        on:focus={() => handleoverAspect([id, ...links], id)}
        on:mouseleave={handleleaveAspect}
        on:blur={handleleaveAspect}
        class:isActive={activeNodes.includes(id) ||
